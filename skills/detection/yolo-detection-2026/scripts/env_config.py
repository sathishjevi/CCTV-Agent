...
def _is_raw_transformers_onnx(onnx_path: str) -> bool:
    """Detect a raw HuggingFace-transformers-style detection export (e.g.
    RT-DETR: input 'pixel_values', outputs 'logits' + 'pred_boxes') as
    opposed to an ultralytics-native export (single fused output tensor,
    input usually named 'images').

    Why this matters: ultralytics' YOLO()/RTDETR() loaders expect their
    OWN export format and silently return zero detections — no error —
    against a raw transformers-style export. This was caught by running
    real video through the pipeline for the first time (every earlier
    phase used synthetic/stub detections, which never exercised real
    inference); see the skill's SKILL.md changelog. Detecting the actual
    format from the file itself (rather than assuming by backend) means
    this fix applies wherever the model is actually loaded — the bug
    previously only had a fix path for the mps/CoreML backend."""
    try:
        import onnxruntime as ort
        sess = ort.InferenceSession(onnx_path, providers=["CPUExecutionProvider"])
        input_names = {i.name for i in sess.get_inputs()}
        output_names = {o.name for o in sess.get_outputs()}
        return "pixel_values" in input_names and {"logits", "pred_boxes"} <= output_names
    except Exception:
        return False
...
class HardwareEnv:
    ...
    def _load_onnx_coreml(self, onnx_path: str):
        ...
        """
        return self._load_onnx_transformers_style(
            onnx_path, providers=['CoreMLExecutionProvider', 'CPUExecutionProvider'])

    def _load_onnx_cpu(self, onnx_path: str):
        """Same wrapper as _load_onnx_coreml, but CPU-only providers — used
        on the cpu/default backend when the .onnx file is detected (via
        _is_raw_transformers_onnx) to be a raw transformers-style export
        that ultralytics' own loader can't decode. See that function's
        docstring for why this exists."""
        return self._load_onnx_transformers_style(onnx_path, providers=['CPUExecutionProvider'])

    def _load_onnx_transformers_style(self, onnx_path: str, providers: list):
        """Shared implementation for _load_onnx_coreml/_load_onnx_cpu —
        only the ONNX Runtime execution providers differ between them.
        """
        import onnxruntime as ort

        session = ort.InferenceSession(onnx_path, providers=providers)
        active = session.get_providers()
        _log(f"ONNX session ({onnx_path}): providers={active}")
    ...
    def _load_onnx_auto(self, onnx_path: str):
        """Choose the right ONNX loader for a given file: CoreML EP on mps,
        otherwise auto-detect whether the file is a raw transformers-style
        export (needs the custom decoder) or an ultralytics-native export
        (use ultralytics' own loader) — see _is_raw_transformers_onnx."""
        if self.backend == "mps":
            return self._load_onnx_coreml(onnx_path)
        if _is_raw_transformers_onnx(onnx_path):
            return self._load_onnx_cpu(onnx_path)
        from ultralytics import YOLO
    ...
    def load_optimized(self, model_name: str, use_optimized: bool = True):
        ...
        if use_optimized and self.framework_ok:
            ...
            if optimized_path.exists():
                try:
                    if self.export_format == "onnx":
                        model = self._load_onnx_auto(str(optimized_path))
                    else:
                        from ultralytics import YOLO
            ...
            if exported:
                try:
                    if self.export_format == "onnx":
                        model = self._load_onnx_auto(str(exported))
                    else:
                        from ultralytics import YOLO
